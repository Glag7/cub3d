...
static void	update_value(t_slider *slider, size_t x, size_t i)
{
	double	v_curr;
...
static int	update_slider(t_slider *slider, size_t x, size_t i)
{
	double	v_curr;
	double	old_v_curr;
	...
	update_value(slider, (size_t)x, i);
	if (slider->type == UINT)
		v_curr = *(uint32_t *)slider->dst;
	else
		v_curr = *(double *)slider->dst;
...
void	update_sliders(t_data *data)
{
	...
	while (i < NB_SLIDERS)
	{
		if (data->menu.sliders[i].state == PRESS
			&& update_slider(&data->menu.sliders[i], x, i) == 1)
		{
