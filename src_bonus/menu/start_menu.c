...
/*   Updated: 2024/09/07 19:46:49 by ttrave           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_main_menu(t_mlx *mlx, t_menu *menu, t_set *set)
{
	...
	draw_string(mlx, set, (t_ulpoint){.x = part(set->wid, 0.5), .y = part(set->hei, 0.15)}, menu->title);
	if (menu->resume == 0)
...
void	draw_exit_menu(t_mlx *mlx, t_menu *menu, t_set *set)
{
	...
	draw_string(mlx, set, (t_ulpoint){.x = part(set->wid, 0.5), .y = part(set->hei, 0.45)}, menu->confirm);
